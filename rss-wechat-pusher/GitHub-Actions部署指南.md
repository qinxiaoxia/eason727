...
- **定时执行**：见 workflow 内 `schedule`（轮巡 cron + 9:30/15:30 定时 cron）；轮巡推实时两类，定时推其余四类
...
**Q：如何修改推送时间？**  
A：编辑 `.github/workflows/rss-push.yml`，修改 `cron`。当前为 **`0 22,0,2,4,6,8,10,12 * * *`**（轮巡 UTC）+ **`30 1,7 * * *`**（定时 UTC），对应北京轮巡 6:00–20:00 每两小时、定时 **9:30/15:30**。同时保持 `config.example.py` 中 `POLL_HOURS_BEIJING` / `SCHEDULED_PUSH_TIMES` 一致。
