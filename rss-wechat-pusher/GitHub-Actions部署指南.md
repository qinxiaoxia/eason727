...
- **定时执行**：见 workflow 内 `schedule`（双线 cron + 9:30/15:30 北京等）；监管预警、重大安全事件→可实时推送
...
**Q：如何修改推送时间？**  
A：编辑 `.github/workflows/rss-push.yml`，修改 `cron`。当前为**两条 schedule**：`30 */3 * * *`（每 3 小时）+ `30 1,7,13,19 * * *`（补强 UTC 1:30/7:30/13:30/19:30，含北京 **9:30/15:30** 等窗口）。
